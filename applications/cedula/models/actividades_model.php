...
class Actividades_model extends CI_Model 
{
    ...
    function get_cal_act($e_mail,$grupo,$id_coord,$edicion)
    {
        switch ($grupo) {
              case 'administrador':
                $this->db->select('id_act,actividad,descripcion,is_costo_publico,sede,ubicacion,quienpropone,d1,d2,d3,d4,d5,d6,d7,d8,d9,d10,hora_ini,hora_fin,id_categoria');
                $this->db->where('id_fc', $edicion);
                $query = $this->db->get(ACTIVIDADES);        
                break;
              case 'coordinador':
...
///////////////////////////////////////////////////
    ...
    function get_master_diario($edicion = null, $fechaPrograma = null, $marca)
    {
        switch ($marca) {
            case '0':
                if ( isset($edicion) AND $id_categoria == null ) {
